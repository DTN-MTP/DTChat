    ...
    pub fn from_env_or_default() -> Self {
        #[cfg(feature = "ack-delay")]
        {
            let ack_delay = match std::env::var("DTCHAT_ACK_DELAY") {
                Ok(delay_str) => match delay_str.parse::<u64>() {
                    Ok(delay) => {
                        println!("✅ Délai ACK lu depuis DTCHAT_ACK_DELAY: {delay}ms");
                        delay
                    }
                    Err(_) => {
                        println!("⚠️  Valeur DTCHAT_ACK_DELAY invalide {delay_str}, utilisation de la valeur par défaut: 100ms");
                        100
                    }
                },
                Err(_) => {
...
pub fn initialize_ack_config() {
    ...
    #[cfg(feature = "ack-delay")]
    println!(
        "✅ Délai ACK configuré: {}ms - {}ms (aléatoire)",
