...
pub fn initialize_ack_config() {
    ...
    let min_delay = match io::stdin().read_line(&mut input) {
        Ok(_) => {
            input.trim().parse::<u64>().unwrap_or(50)
        }
        Err(_) => {
    ...
    let max_delay = match io::stdin().read_line(&mut input) {
        Ok(_) => {
            ...
            if parsed < min_delay {
                println!("⚠️  Maximum ({}) < minimum ({}), ajustement automatique à {}ms", 
                        parsed, min_delay, min_delay + 50);
                min_delay + 50
            } else {
                parsed
            }
    ...
    println!("✅ Délai ACK configuré: {}ms - {}ms (aléatoire)", min_delay, max_delay);
}
